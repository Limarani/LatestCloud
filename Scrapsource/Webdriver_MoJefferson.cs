        ...
        public string FTP_Jefferson(string streetno, string direction, string streetname, string city, string streettype, string unitnumber, string ownernm, string parcelNumber, string searchType, string orderNumber, string directParcel)
        {
            ...
            using (driver = new PhantomJSDriver())
                try
                {
                    ...
                    int k = 0;
                    string Tax_Amount="", PaidAmount="", TotalDue = "", strtaxyear=""; 
                    ...
                    foreach (IWebElement Payment in PaymentHistoryrow)
                    {
                        ...
                        if (PaymentHistoryid.Count != 0)
                        {
                            ...
                            strtaxyear = PaymentHistoryid[0].Text;
                            amck.TaxId = strtaxyear;
                            Tax_Amount = PaymentHistoryid[1].Text;
                            amck.Instamount1 = Tax_Amount;
                            PaidAmount = PaymentHistoryid[2].Text;
                            amck.Instamountpaid1 = PaidAmount;
                            TotalDue = PaymentHistoryid[3].Text;

                            if(k==0)
                            {
                                if(Tax_Amount== PaidAmount)
                                {
                                    amck.InstPaidDue1 = "Paid";
                                    amck.IsDelinquent = "No";
                                }
                                if (Tax_Amount == TotalDue)
                                {
                                    amck.InstPaidDue1 = "Due";
                                    amck.IsDelinquent = "No";
                                }
                                if (PaidAmount!="$0.00" && Tax_Amount != PaidAmount)
                                {
                                    amck.IsDelinquent = "Yes";
                                }
                                if (TotalDue !="$0.00" && Tax_Amount != TotalDue)
                                {
                                    amck.IsDelinquent = "Yes";
                                }
